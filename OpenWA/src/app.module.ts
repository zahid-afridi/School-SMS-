...
// Explicit entity imports — TypeORM path globs break on Windows (glob treats `\` as escape).
...
const MAIN_ENTITIES = [ApiKey, AuditLog];
const DATA_ENTITIES = [
  Session,
  Message,
  MessageBatch,
...
/** TypeORM glob paths must use forward slashes on Windows. */
function typeormGlob(...segments: string[]): string {
  return path.join(__dirname, ...segments).replace(/\\/g, '/');
}
    ...
    TypeOrmModule.forRootAsync({
      ...
      useFactory: (configService: ConfigService) => {
        ...
        return {
          ...
          migrations: [typeormGlob('database', 'migrations-main', '*{.ts,.js}')],
          synchronize,
    ...
    TypeOrmModule.forRootAsync({
      ...
      useFactory: (configService: ConfigService) => {
        ...
        const baseConfig = {
          ...
          autoLoadEntities: true,
          migrations: [typeormGlob('database', 'migrations', '*{.ts,.js}')],
