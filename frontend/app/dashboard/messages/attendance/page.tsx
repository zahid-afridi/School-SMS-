...
import { useSendAttendanceWhatsAppMutation } from "@/redux/features/messages/messageApi";

export default function AttendanceMessagePage() {
  ...
  const [send, { isLoading }] = useSendAttendanceWhatsAppMutation();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Attendance notice sent");
    } catch (err: unknown) {
      toast.error(
      ...
      <div className="max-w-3xl mx-auto">
        ...
        >
          ...
          <div className="rounded-xl bg-slate-50 border border-slate-200 p-4 text-sm text-slate-700">
            ...
            Dear Parent, your child {previewName} was marked absent today.
          </div>

          <button
            type="submit"
            disabled={isLoading}
            className="inline-flex items-center justify-center px-5 py-2.5 rounded-xl bg-orange-600 text-white font-semibold hover:bg-orange-700 disabled:opacity-60"
          >
            {isLoading ? "Sending…" : "Send Absent Notice"}
          </button>
        </form>
      </div>
    </div>
