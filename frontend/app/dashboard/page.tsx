  ...
  const content = (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow h-full">
      <div className="flex items-center justify-between">
        ...
        <div className={`p-4 rounded-full ${tones[tone]}`}>{icon}</div>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link href={href} className="text-sm text-emerald-700 font-medium hover:underline">
          View
        </Link>
      </div>
      <div className="grid grid-cols-2 gap-3 text-sm mb-4">
        <div className="rounded-xl bg-emerald-50 px-3 py-2">
          ...
          <p className="font-bold text-lg">{summary.present}</p>
        </div>
        <div className="rounded-xl bg-amber-50 px-3 py-2">
          <p className="text-amber-700 text-xs">Leave</p>
          <p className="font-bold text-lg">{summary.leave}</p>
        </div>
        <div className="rounded-xl bg-rose-50 px-3 py-2">
          <p className="text-rose-700 text-xs">Absent</p>
          <p className="font-bold text-lg">{summary.absent}</p>
        </div>
      ...
      <p className="text-sm text-slate-500">
        Attendance rate:{" "}
        <span className="font-semibold text-slate-800">{summary.percentage}%</span>
      </p>
    </div>
  );
}
  ...
  if (isError || !data) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] gap-3">
        <p className="text-rose-500">Failed to load dashboard.</p>
        <button
          type="button"
          onClick={() => refetch()}
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            {data.school.name} · Live overview of students, staff, fees, and attendance
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-semibold"
          >
            <FaHandHoldingUsd size={14} /> Collect Fees
          </Link>
          {isFetching ? (
            <span className="text-xs text-slate-400 self-center">Refreshing…</span>
          ) : null}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
          ...
          <div className="space-y-2 text-sm">
            <QuickLink href="/dashboard/attendance/students/mark" label="Mark student attendance" />
            <QuickLink href="/dashboard/fees/generate" label="Generate monthly fees" />
            <QuickLink href="/dashboard/reports" label="Open reports" />
            <QuickLink href="/dashboard/exams" label="Manage exams" />
            <QuickLink href="/dashboard/students/promote" label="Promote students" />
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
          <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
            <h3 className="font-semibold text-slate-900">Recent Fee Payments</h3>
            <Link
              href="/dashboard/reports/fee-collection"
                ...
                <tbody>
                  {data.recentPayments.map((payment) => (
                    <tr key={payment.id} className="border-t border-slate-100">
                      ...
                      <td className="px-4 py-2.5">{payment.receiptNo}</td>
                      <td className="px-4 py-2.5">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
          ...
          <div className="px-5 py-3 border-t border-slate-100 text-sm text-slate-500">
            {data.exams.active} active exam(s) · {data.counts.sections} section(s) ·{" "}
            {data.counts.parents} parent(s)
          </div>
        </div>
      </div>
    </>
